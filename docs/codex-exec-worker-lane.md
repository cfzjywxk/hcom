...
real CI are the last word. hcom neither pushes nor installs, so a wrong judgment
costs a review round or a local commit, both cheap and revertible.
...
amendment-aware review contract, the human reads the final report, and real CI
runs after a push. hcom neither pushes nor installs, so the worst case is a
wasted review round or a local commit to revert — both cheap.
