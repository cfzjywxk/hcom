...
fn fork_version_help_and_status_use_human_visible_version() {
    ...
    assert_eq!(code, 0);
    assert_eq!(stdout, "hcom 1.0.19\n");
    assert!(stderr.is_empty(), "stderr={stderr}");

    let (code, stdout, stderr) = h.run(["--help"]);
    assert_eq!(code, 0, "stderr={stderr}");
    assert!(
        stdout.starts_with("hcom (hook-comms) v1.0.19 "),
        "stdout={stdout}"
    );
    assert!(
    ...
    assert_eq!(code, 0, "stderr={stderr}");
    assert_eq!(stdout.lines().next(), Some("hcom 1.0.19"));
    assert!(stderr.is_empty(), "stderr={stderr}");
    ...
    assert_eq!(status["version"]["current"], "1.0.19");
