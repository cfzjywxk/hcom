...
files, including SSH and cloud credentials, remain writable. Exact current-user
...
unchanged. Retained CLI reviewers keep their documented read-only source view.
Each authorized task names the exact canonical Git root discovered by the
Architect from project documentation. Developer tasks commit directly there;
hcom has no repository-root allowlist. An in-scope uncommitted developer result
...
lifecycle internally without model calls, so short-cadence polling only wastes
Architect requests and tokens.
