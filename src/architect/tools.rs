...
Every review is one strict synchronous generation across the ordered Reviewer1 \
and Reviewer2 lanes. `review_round` counts only generations whose two logical \
...
`review_responded` event. The first Reviewer response is only partial progress: \
do not report that the review cycle finished, do not start or imply a Developer \
correction, and immediately re-arm `session_wait` for the peer response. Never \
read or summarize a Reviewer final path merely to display progress. Only after \
terminal, read both Reviewers' listed current-generation evidence and report \
their original verdicts/findings. A task is LGTM only when Reviewer1 and \
Reviewer2 both returned LGTM for that same generation; do not create or request \
a post-LGTM commit.
            ...
            [
                ...
                (
                    ...
                    json!({
                        "type":"boolean",
                        "const":true,
                        "description":"Attest that the human explicitly authorized execution by approving this exact displayed draft, by directing the Architect to follow/implement/execute a named existing detailed plan/specification/current_todo, or by directing the Architect to plan/define the solution and then implement/proceed/finish/drive the requested work. A faithful derived plan may use the last form in the same turn when it adds no unresolved material decision. Standard-lane execution includes one local signed-off candidate commit per task and same-commit amendments, but not push/install/release."
                    }),
                ),
            ],
        ),
        ActionName::SessionClarificationSubmit => object_schema(
            ...
            [
                ...
                (
                    ...
                    json!({
                        "type":"boolean",
                        "description":"False only for a permitted Architect-derived clarification. A conflict between an explicit no-commit instruction and the standard lane's required candidate commit must first use session_clarification_require_human, then submit true only after the human actually resolves it."
                    }),
                ),
            ],
        ),
...
mod tests {
    ...
    fn execution_authorization_contract_supports_exact_named_or_prospective_start() {
        ...
        assert!(plan.contains("synchronized dual-Reviewer generation budget"));
        assert!(plan.contains("Reviewer1 and Reviewer2 concurrently"));
    ...
    fn worker_status_contract_uses_latched_action_wait_without_model_polling() {
        ...
        assert!(wait.contains("first response is partial progress"));
        assert!(wait.contains("peer response is still pending"));
        assert!(wait.contains("both Reviewers' listed current-generation final files"));
        assert!(wait.contains("same-generation Reviewer1 and Reviewer2 LGTM"));
    ...
    fn architect_instructions_and_clarification_tools_keep_role_and_human_boundaries() {
        for required in [
            ...
            "Only after terminal",
            "Reviewer1 and Reviewer2 both returned LGTM",
        ] {
            assert!(
