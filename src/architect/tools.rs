...
fn tool_description(action: ActionName, github_pr: bool) -> &'static str {
    match action {
        ...
        ActionName::SessionGitHubDeliveryInspect => {
            "Refresh the exact read-only private repository, base ref/SHA, complete active-App permission maps, identities, installations, and canonical hcom-critical ruleset attestation for the current GitHub Pull Request run. Bind run_id and expected_session_version to the current snapshot. This operation creates no ref, worktree, branch, Pull Request, Check, comment, or token-bearing artifact and does not advance session.version. Only its newest inspection_id can be consumed by session_plan_replace; a later inspection supersedes an earlier one even when the observed SHA is unchanged. It is available only in explicit --github-pr mode."
        }
        ActionName::SessionPlanReplace if github_pr => {
            "Bind the bounded ordered GitHub Pull Request task plan using file pointers and the exact newest github_inspection_id. Every task repository_root must equal the frozen canonical local repository root. Display the complete delivery/run binding and every existing task field, material assumption, plan version, and plan hash before approval. The plan hash binds the frozen delivery profile, inspected base/ref/ruleset, inspection ID, and deterministic generated branch. This call creates no branch, worktree, Pull Request, Check, comment, worker, or other GitHub write. Every Developer turn must append exactly one new signed-off commit; corrections and later-task initial turns never amend, rebase, squash, reword, or force-push published history. Approval does not authorize install, release, deployment, or unrelated repository mutation."
        }
        ...
        ActionName::SessionApproveAndStart if github_pr => {
            "Start the exact displayed GitHub Pull Request plan only with valid human execution authorization. Bind expected_session_version, plan_version, and plan_hash exactly. Approval authorizes the frozen one-PR workflow for this run—base fetch, one managed worktree/branch, append-only task commits and publication, ordered active-App review/check publication, and an exact-head squash merge only after every task is LGTM and all gates pass. It does not authorize install, release, package publication, deployment, another repository/branch, force-push, or any unrelated mutation. On Running, immediately use session_wait; do not poll."
        }
        ...
        ActionName::SessionWait if github_pr => {
            "Passively wait within the exact current run_id for one retained GitHub delivery progress event, a terminal session, or a latched pending_architect_action. This is event-driven, not polling. Events may be candidate_published, review_requested, review_responded, task_completed, merge_waiting, or run_finalizing. Display each event once with its exact PR URL, published/final head SHA, task position and generation when task-scoped, and Reviewer identity/verdict/response counts when review-scoped; a partial dual response must immediately re-arm this wait. Do not read or summarize native final files merely for progress. Preserve the existing clarification escalation contract. Queued progress must drain before terminal. At terminal, read the listed current-generation native finals and report the exact PR, per-task ranges/outcomes, ordered Reviewer review URLs/verdicts, hcom/review Check, approved and final ruleset evidence, delivery outcome, merge SHA when delivered, and preserved branch/worktree/PR otherwise. Cancellation or interruption of this tool never cancels the run."
        }
