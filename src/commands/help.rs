...
\n\
Commands:\n\
  arch         Launch a blank in-memory session-task architect\n\
...
pub fn get_command_help(name: &str) -> String {
    ...
    let entries: Option<&[HelpEntry]> = match name {
        "arch" => return hcom::architect::help_text().to_owned(),
        "list" => Some(LIST_HELP),
        "send" => Some(SEND_HELP),
...
mod tests {
    ...
    fn help_text_contains_version() {
        assert!(get_help_text().starts_with("hcom (hook-comms) v1.0.05 "));
    }
